...
# input integer. Coins are based on a system as follows: there are coins representing the integers 
# 1, 5, 7, 9, and 11. So for example: if num is 16, then the output should be 2 because you can achieve 
...
def CoinDeterminer(num)
  coins = [1,5,7,9,11]
  if coins.include?(num)
    return 1
  end
  coincount = []
  coins.each do |value|
    if num > value
      coincount << (1 + (CoinDeterminer(num - value)))
    end
  end
  coincount.min
end
